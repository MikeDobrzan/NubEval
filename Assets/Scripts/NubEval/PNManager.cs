...
public class PNManager : PNManagerBehaviour {
	...
	private async void Awake() {
		...
		// Subscribe example
		pubnub.Subscribe<string>().Channels(new[] { "Channel-Barcelona" }).Execute();

		// Publish example
		await pubnub.Publish().Channel("Channel-Barcelona").Message("Hello World from Unity!").ExecuteAsync();
	}
